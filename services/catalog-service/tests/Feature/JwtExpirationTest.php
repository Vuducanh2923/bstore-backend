...
test('JWT with exp equal to now is expired', function () {
    ...
    $key = 'catalog-jwt-test-key-at-least-32-bytes';
    config(['auth.token_key' => $key]);
    $token = catalogBoundaryJwt($key, Carbon::now()->timestamp);
    $request = Request::create('/api/products', 'GET', server: ['HTTP_AUTHORIZATION' => 'Bearer '.$token]);
