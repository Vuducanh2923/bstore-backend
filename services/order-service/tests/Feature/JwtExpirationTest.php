...
test('JWT with exp equal to now is expired', function () {
    ...
    $key = 'order-jwt-test-key-at-least-32-bytes';
    config(['auth.token_key' => $key]);
    $token = orderBoundaryJwt($key, Carbon::now()->timestamp);
    $request = Request::create('/api/orders', 'GET', server: ['HTTP_AUTHORIZATION' => 'Bearer '.$token]);

    expect(app(AuthTokenService::class)->payloadFromRequest($request))->toBeNull();
});

// Thực hiện đơn hàng boundary JWT.
function orderBoundaryJwt(string $key, int $exp): string
{
